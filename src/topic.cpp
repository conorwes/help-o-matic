...
void add_styled_section(XMLNode *parent, const std::string &title, const std::string &placeholder_text = g_placeholder)
{
...
void add_collapsable_section(XMLNode *parent, const std::string &text, const std::string &toggle_id, const std::string &placeholder_text = g_placeholder)
{
    ...
    auto *a = add_element(toggle, "a");
    a->SetAttribute("class", "dropdown-toggle");
    a->SetAttribute("style", "font-style: normal;font-weight: normal;color: #000000;background-color: transparent;text-decoration: none;");
...
{
    std::string f = std::move(filename);
    std::replace(f.begin(), f.end(), '.', '-');

    const auto it = g_type_map.find(m_type);
    const path target_dir = (it != g_type_map.end()) ? it->second.dir : g_autogen_dir;
...
auto Topic::get_topic_name() const -> const std::string &
{
    return m_topic_name;
}

auto Topic::get_keyword() const -> const std::string &
{
    return m_keyword;
}

auto Topic::get_filename() const -> const std::string &
{
    return m_filename;
}

auto Topic::get_topic_type() const noexcept -> TopicType
{
    return m_type;
}

auto Topic::create_topic(const std::string &prev_topic, const std::string &next_topic) -> bool
{
    ...
    const std::string display_name = m_topic_name + (g_type_map.count(m_type) ? g_type_map.at(m_type).suffix : "");
    ...
    script_1->InsertEndChild(doc.NewText(""));
    ...
    script_2->InsertEndChild(doc.NewText(""));
    ...
    script_3->InsertEndChild(doc.NewText(""));
    ...
    const std::string id = "toggle0186a";
    int id_suffix = 1;
    int num_toggles = 0;
    ...
    {
        const auto toggle_id = id + std::to_string(i + 1);
    ...
    const std::string cdata_content = "*/\n" + script_text + "\n/*";
    ...
    std::ofstream out_file(m_filename.c_str(), std::ios::out | std::ios::trunc);
    if (!out_file)
    {
        std::cerr << "Failed to write topic file: " << m_filename << '\n';
        return false;
    }
    out_file << printer.CStr();
